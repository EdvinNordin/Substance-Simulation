...
public class WE_GPU : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        heightTexture = new RenderTexture(planeWidth, planeHeight, 0, RenderTextureFormat.RFloat) { enableRandomWrite = true };
        heightTexture.Create();

        velTexture = new RenderTexture(planeWidth, planeHeight, 0, RenderTextureFormat.RFloat) { enableRandomWrite = true };
        velTexture.Create();
        
        accTexture = new RenderTexture(planeWidth, planeHeight, 0, RenderTextureFormat.RFloat) { enableRandomWrite = true };
        accTexture.Create();
    ...
    void Update()
    {
        computeShader.Dispatch(WaveEquationKernel, planeWidth/10, planeHeight/10, 1); //10=FOV scaling
    ...
    public static int GetClosestVertex(RaycastHit aHit, int[] aTriangles)
    {
        ...
        int index = aHit.triangleIndex * 3; //3=FOV scaling
