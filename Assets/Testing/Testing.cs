...
public class Testing : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        densityTexture = new RenderTexture(20, 20, 0, RenderTextureFormat.RFloat);
        densityTexture.enableRandomWrite = true;
        densityTexture.Create();

        densityPrevTexture = new RenderTexture(20, 20, 0, RenderTextureFormat.RFloat); 
        densityPrevTexture.enableRandomWrite = true;
        densityPrevTexture.Create();

        velocityTexture = new RenderTexture(20, 20, 0, RenderTextureFormat.RGFloat);
        velocityTexture.enableRandomWrite = true;
        velocityTexture.Create();

        velocityPrevTexture = new RenderTexture(20, 20, 0, RenderTextureFormat.RGFloat); 
        velocityPrevTexture.enableRandomWrite = true;
        velocityPrevTexture.Create();

        velocityTempTexture = new RenderTexture(20, 20, 0, RenderTextureFormat.RGFloat);
        velocityTempTexture.enableRandomWrite = true;
        velocityTempTexture.Create();

        pressureTexture = new RenderTexture(20, 20, 0, RenderTextureFormat.RFloat);
        pressureTexture.enableRandomWrite = true;
        pressureTexture.Create();

        pressurePrevTexture = new RenderTexture(20, 20, 0, RenderTextureFormat.RFloat);
        pressurePrevTexture.enableRandomWrite = true;
        pressurePrevTexture.Create();

        divergenceTexture = new RenderTexture(20, 20, 0, RenderTextureFormat.RFloat);
        ...
        //navierStokesShader.Dispatch(addValueKernel, 20, 20, 1);
    }
    ...
    void Update()
    {
        ...
        /*navierStokesShader.SetTexture(addValueKernel, "Out", densityPrevTexture);
        navierStokesShader.Dispatch(addValueKernel, 20, 20, 1);*/
    }
        ...
        if (Input.GetMouseButton(1))
        {
            if (Physics.Raycast(ray, out hit))
            {
                ...
                else
                {
                    ...
                    navierStokesShader.Dispatch(addVelocityKernel, 20, 20, 1);
                }
            }
        }
    }
    ...
    void Diffuse(RenderTexture inTexture, RenderTexture outTexture, bool setBounds = false)
    {
            ...
            navierStokesShader.Dispatch(diffusionKernel, 20, 20, 1);
    ...
    void Advect(RenderTexture inTexture, RenderTexture outTexture, bool setBounds = false)
        {
            ...
            navierStokesShader.Dispatch(advectionKernel, 20, 20, 1);
            if (setBounds) SetBounds(inTexture, outTexture);
        }
    ...
    void Projection()
    {
        ...
        navierStokesShader.Dispatch(projectionKernel,20, 20, 1);
        ...
        navierStokesShader.Dispatch(projection2Kernel,20, 20, 1);
        ...
        navierStokesShader.Dispatch(projection3Kernel, 20, 20, 1);
        SetBounds(velocityPrevTexture, velocityTexture);
    }
